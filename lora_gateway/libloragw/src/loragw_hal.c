...
int lgw_receive(uint8_t max_pkt, struct lgw_pkt_rx_s *pkt_data) {
	...
	clock_gettime(CLOCK_REALTIME, &ts_spec);
        ...
        // Test
        printf("ch:%02i Freq:%03.2f Rssi:%04.0f ", p->rf_chain, (float)p->freq_hz/1000000, p->rssi - RSSI_MULTI_BIAS);
        ...
        if ((ifmod == IF_LORA_MULTI) || (ifmod == IF_LORA_STD)) {
            ...
            // Test
            printf("snr:%06.2f min_snr:%06.2f max_snr:%06.2f ", p->snr, p->snr_min, p->snr_max);
            ...
            sf = (buff[sz+1] >> 4) & 0x0F;
            switch (sf) {
                case 7: p->datarate = DR_LORA_SF7; printf("sf: 7 "); break;
                case 8: p->datarate = DR_LORA_SF8; printf("sf: 8 "); break;
                case 9: p->datarate = DR_LORA_SF9; printf("sf: 9 "); break;
                case 10: p->datarate = DR_LORA_SF10; printf("sf:10 "); break;
                case 11: p->datarate = DR_LORA_SF11; printf("sf:11 "); break;
                case 12: p->datarate = DR_LORA_SF12; printf("sf:12 "); break;
                default: p->datarate = DR_UNDEFINED; printf("sf:err ");
            }
            cr = (buff[sz+1] >> 1) & 0x07;
            switch (cr) {
        ...
        printf("crc:%04X ", p->crc);
        printf("tmsp:%010u ", raw_timestamp);
        printf("diff:%06u ", raw_timestamp - raw_timestamp_old);
        raw_timestamp_old = raw_timestamp;
        printf("mID:%02X ", buff[sz+12]);
        //printf("rxMaxBinPos:%03i ", (uint16_t)buff[sz+13] + ((uint16_t)buff[sz+14] << 8) );
        printf("rxCorrSNR:%02i ", buff[sz+15]);
